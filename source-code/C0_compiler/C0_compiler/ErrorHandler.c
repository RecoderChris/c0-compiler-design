...
void errormsg(int options) {
	...
	case VOID_RETURN:printf("Error:(ll: %d,cc: %d) It is a function without return value.\n",ll,cc);
		break;
	case CALL_PARANUM_ERROR:printf("Error:(ll: %d,cc: %d) Calling parameter is not equal to the pre-define value.\n",ll,cc);
		break;
	case CALL_PARATYPE_ERROR:printf("Error:(ll: %d,cc: %d) Calling parameter's type is not right in this position.\n",ll,cc);
		break;
	case NO_PARA_WITH_PARENT:printf("Error:(ll: %d,cc: %d) Calling error.No parameter,but there is a lparent.\n",ll,cc);
		break;
	case VOID_WITH_RETURN:printf("Error:(ll: %d,cc: %d) Void function with a return value.\n", ll, cc);
		break;
	case D_QUOTE_NOMATCH:printf("Error:(ll : %d, cc : %d) D-quote is not match.\n",ll,cc);
		break;
	case ID_CONFLICT:printf("Error:(ll : %d, cc : %d) ID conflict with int or char.\n", ll, cc);
		break;
	case ARR_OUT_RANGE:printf("Error:(ll : %d, cc : %d) Array is out of range.\n", ll, cc);
		break;
	case TYPE_ERROR:printf("Error:(ll : %d, cc : %d) The type is not match.\n", ll, cc);
		break;
...
void errorhandle() {
	while (!CmpString(SymType, SYM[SEMICOLON].symbol, MAX_SYM_LENGTH)&&
		!CmpString(SymType, SYM[RBRACE].symbol, MAX_SYM_LENGTH)) {
		InSym();
	}
}
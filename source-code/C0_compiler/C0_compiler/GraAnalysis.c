...
int Judged = 0,pos = 0;
char itemname[MAX_ID_LENGTH] = "                         ", itemtype[MAX_SYM_LENGTH] = "VOID                     ", itemkind[MAX_SYM_LENGTH] = "NOTYPE                   ", itemtmp = 0;
...
int void_flag = 0;
int exp_no = 0;
int static_array_sub = -1,process_array = 0;
char exp_type[MAX_ID_LENGTH] = "INT                      ";
...
void expression() {
	...
	if (neg) {
		CpyString(exp_type, "INT                      ", MAX_OP_LEN);
		emit(op[NEG], tmp, tmp, op[EMPTY]);
		found = find_sym(tmp);
		CpyString(C0_SymTable.item[found].type, "INT                      ", MAX_OP_LEN);
	}
	...
	if (neg) {
		static_array_sub = 0-static_array_sub;
	}
	if (have_plus) {
		static_array_sub = -1;
	}
...
void factor() {
	...
	if (CmpString(SymType, SYM[IDENT].symbol, MAX_SYM_LENGTH)) {
		...
		if (CmpString(SymType, SYM[LBRACK].symbol, MAX_SYM_LENGTH)) {//array
			...
			if (C0_SymTable.item[found].number == 0) {
				errormsg(GET_A_WRONG_ARR);
				errorhandle();
				return;
			}
			InSym();
			expression();
			found1 = find_sym(exp_name);
		...
		if (CmpString(SymType, SYM[INTCON].symbol, MAX_SYM_LENGTH)) {
			...
			CpyString(itemname, factor_name, MAX_SYM_LENGTH);
			CpyString(itemtype, "INT                      ", MAX_SYM_LENGTH);
			CpyString(itemtype, exp_type, MAX_SYM_LENGTH);
			CpyString(itemkind, "VARIABLE                 ", MAX_SYM_LENGTH);
			itemtmp = 1;
			if (entertable()) {
				errormsg(SYM_REDEFINE);
		...
		InSym();
		if (CmpString(SymType, SYM[RBRACK].symbol, MAX_SYM_LENGTH) && (process_array)) {
			static_array_sub = NumValue;
		}
		return;
	}
		...
		if ((StringValue[0] >= '0') && (StringValue[0] <= '9')){
			d = StringValue[0];
			num2str(d);
			CpyString(StringValue, num10, MAX_OP_LEN);
		}
...
void assignstate() {
	...
	if (CmpString(SymType, SYM[BECOME].symbol, MAX_SYM_LENGTH)) {
		...
		if (CmpString(C0_SymTable.item[found].type, "INT                      ", MAX_ID_LENGTH) &&
			CmpString(C0_SymTable.item[found1].type, "CHAR                     ", MAX_ID_LENGTH)) {
			errormsg(TYPE_ERROR);
			errorhandle();
			return;
		}
...
void condition(char label[]) {
	...
	expression();
	found1 = find_sym(exp_name);
	if (CmpString(C0_SymTable.item[found1].type, "CHAR                     ", MAX_ID_LENGTH)) {
		errormsg(TYPE_ERROR);
		errorhandle();
		return;
	}
...
＜情况子语句＞  ::=  case＜常量＞：＜语句＞
*/
void casetable(char *defaultlabel,int found) {
	int neg = 0;
	...
	if (CmpString(SymType, SYM[CASESY].symbol, MAX_SYM_LENGTH)) {
		...
		if (CmpString(SymType, SYM[INTCON].symbol, MAX_SYM_LENGTH) |
			...
			if (CmpString(SymType, SYM[INTCON].symbol, MAX_SYM_LENGTH)) {
				if (!CmpString(C0_SymTable.item[found].type, "INT                      ", MAX_SYM_LENGTH)) {
					errormsg(TYPE_ERROR);
					errorhandle();
					return;
				}
	...
	if (CmpString(SymType, SYM[RETURNSY].symbol, MAX_SYM_LENGTH)) {
		...
		if (CmpString(SymType, SYM[LPARENT].symbol, MAX_SYM_LENGTH)) {
			if (void_flag) {
				...
				expression();
				found = find_sym(exp_name);
				if (!CmpString(C0_SymTable.item[found].type, C0_SymTable.item[curlev].type, MAX_ID_LENGTH)) {
					errormsg(TYPE_ERROR);
					errorhandle();
					return;
				}
...
void callstate(char *func_name) {
	...
	if (CmpString(SymType, SYM[LPARENT].symbol, MAX_SYM_LENGTH)) {
		...
		C0_SymTable.item[found1].tmp = 0;
		num2str(counter);
		if (!CmpString(C0_SymTable.item[found + counter].type, C0_SymTable.item[found1].type, MAX_SYM_LENGTH)) {
			errormsg(CALL_PARATYPE_ERROR);
			errorhandle();
		}
		CpyString(p[top].string1, op[PARA], MAX_OP_LEN);
		CpyString(p[top].string2, exp_name, MAX_OP_LEN);
		CpyString(p[top].string3, num10, MAX_OP_LEN);
		CpyString(p[top].string4, func, MAX_OP_LEN);
		top++;
		...
		if (CmpString(SymType, SYM[RPARENT].symbol, MAX_SYM_LENGTH)) {
			InSym();
			for (k = 0; k < top; k++) {
				emit(p[k].string1, p[k].string2, p[k].string3, p[k].string4);
			}
